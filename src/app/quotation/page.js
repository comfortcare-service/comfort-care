...
export default function QuotationGenerator() {
  ...
  // ── State variables ──
  const [docType, setDocType] = useState('Quotation'); // Quotation, Estimate
  const [quotationNo, setQuotationNo] = useState('');
  const [date, setDate] = useState('');
  const [validityDays, setValidityDays] = useState('15'); // 7, 15, 30, Custom
  const [customValidityDate, setCustomValidityDate] = useState('');
  const [subject, setSubject] = useState('Repair & Servicing of Home Appliances');
  ...
  const [items, setItems] = useState([
    { description: 'General Inspection & Diagnosis', price: 150, quantity: 1 }
  ]);
  ...
  const [notes, setNotes] = useState(
    '1. Validity: This quote is valid for 15 days from the date of issue.\n2. Payment: 50% advance along with work order confirmation, balance on completion of work.\n3. Warranty: 3 months warranty on genuine replaced parts.\n4. Disputes: Subject to Mumbai jurisdiction only.'
  );

  // ── Auto-generate quotation number & date on load ──
  useEffect(() => {
    ...
    setQuotationNo(`QC-QT-${yyyy}${mm}${dd}-${randomNum}`);
  }, []);

  // ── Calculate validity date based on selection ──
  const getValidityDateString = () => {
    if (validityDays === 'custom') {
      return customValidityDate ? new Date(customValidityDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Custom';
    }
    if (!date) return 'N/A';
    const baseDate = new Date(date);
    baseDate.setDate(baseDate.getDate() + parseInt(validityDays));
    return baseDate.toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' });
  };

  // ── Update notes validity dynamically when validity selection changes ──
  useEffect(() => {
    if (validityDays !== 'custom') {
      setNotes((prevNotes) => {
        const regex = /1\.\sValidity:\sThis\squote\sis\svalid\sfor\s\d+\sdays/i;
        if (regex.test(prevNotes)) {
          return prevNotes.replace(regex, `1. Validity: This quote is valid for ${validityDays} days`);
        }
        return prevNotes;
      });
    }
  ...
  const handleReset = () => {
    ...
    setSubject('Repair & Servicing of Home Appliances');
    setCustomerName('');
    setCustomerPhone('');
    setCustomerEmail('');
    setCustomerAddress('');
    ...
    setApplianceBrand('');
    setApplianceModel('');
    setItems([{ description: 'General Inspection & Diagnosis', price: 150, quantity: 1 }]);
    ...
    setOtherCharges(0);
    setOtherChargesLabel('Other Charges');
    setNotes(
      '1. Validity: This quote is valid for 15 days from the date of issue.\n2. Payment: 50% advance along with work order confirmation, balance on completion of work.\n3. Warranty: 3 months warranty on genuine replaced parts.\n4. Disputes: Subject to Mumbai jurisdiction only.'
    );
  };
  ...
  const amountBeforeTax = Math.max(0, subtotal - discount) + activeVisitingCharge + (parseFloat(otherCharges) || 0);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <section className="print:hidden lg:col-span-6 space-y-6">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-3xl p-6 sm:p-8 shadow-md border border-gray-100 dark:border-gray-800 transition-colors">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold flex items-center">
                  <FontAwesomeIcon icon={faFileInvoice} className="text-primary-500 mr-2" />
                  Document Settings
                </h3>
                <button
                  onClick={handleReset}
              ...
              {/* Title Selector */}
              <div className="mb-6">
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2">Document Type</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Document Number</label>
                  <input
                    type="text"
                    value={quotationNo}
                    onChange={(e) => setQuotationNo(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-slate-800 focus:outline-none focus:border-primary-500 font-mono"
                    placeholder="QC-QT-20260713-1234"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Date of Issue</label>
                  <input
                    type="date"
                    value={date}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Validity Period</label>
                  <select
                    value={validityDays}
                ...
                {validityDays === 'custom' && (
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Valid Until</label>
                    <input
                      type="date"
              ...
              <div className="mb-6">
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Subject / Scope of Work</label>
                <input
                  type="text"
                  value={subject}
                  onChange={(e) => setSubject(e.target.value)}
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-slate-800 focus:outline-none focus:border-primary-500"
                  placeholder="e.g. Repair, Refilling and Maintenance of Split AC units"
                />
              </div>

              {/* Customer Details */}
              <div className="border-t border-gray-100 dark:border-gray-800 my-6 pt-6">
                <h4 className="font-semibold mb-4 text-sm text-gray-600 dark:text-gray-300">Client Information</h4>
              ...
              {/* Appliance details */}
              <div className="border-t border-gray-100 dark:border-gray-800 my-6 pt-6">
                <h4 className="font-semibold mb-4 text-sm text-gray-600 dark:text-gray-300">Appliance Details</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <input
                    ...
                    placeholder="Brand (e.g. LG, Daikin)"
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-slate-800 focus:outline-none focus:border-primary-500"
                  />
                  <input
                    type="text"
            ...
            <div className="bg-white dark:bg-gray-900 rounded-3xl p-6 sm:p-8 shadow-md border border-gray-100 dark:border-gray-800 transition-colors">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Estimated GST / Tax</label>
                  <select
                    value={gstRate}
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Visiting / Service Fee</label>
                  <div className="flex items-center space-x-3 mt-2">
                    <input
                      type="checkbox"
                      id="waive-visiting-quotation"
                      checked={isVisitingChargeWaived}
                      onChange={(e) => setIsVisitingChargeWaived(e.target.checked)}
                      className="w-5 h-5 rounded text-primary-500 focus:ring-primary-500"
                    />
                    <label htmlFor="waive-visiting-quotation" className="text-sm font-semibold text-gray-600 dark:text-gray-300">
                      Waived / Free
                    </label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 border-t border-gray-100 dark:border-gray-800 pt-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Additional Charges Label</label>
                  <input
                    type="text"
                    value={otherChargesLabel}
                    onChange={(e) => setOtherChargesLabel(e.target.value)}
                    placeholder="e.g. Transportation / Labor"
                    className="w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-slate-800 focus:outline-none focus:border-primary-500 text-xs"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Additional Amount (₹)</label>
                  <input
                    type="number"
                    min="0"
                    value={otherCharges || ''}
                    ...
                    placeholder="0"
                    className="w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-slate-800 focus:outline-none focus:border-primary-500 text-xs font-semibold"
                  />
                </div>
              </div>

              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Terms, Notes &amp; Validity Rules</label>
                <textarea
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  rows={4}
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-slate-800 focus:outline-none focus:border-primary-500 text-sm resize-none font-sans"
                />
              </div>
            </div>
          </section>

          {/* ── RIGHT PANEL: Professional Quotation Letter Preview ── */}
          <section className="lg:col-span-6 space-y-6">
            ...
            <div className="print:hidden bg-white dark:bg-gray-900 rounded-3xl p-6 shadow-md border border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0 transition-colors">
              <div className="text-center sm:text-left">
                <h4 className="font-bold text-lg">Live Quotation Preview</h4>
                <p className="text-xs text-gray-500 dark:text-gray-400">Review layout before saving or printing</p>
              </div>
              <button
                onClick={handlePrint}
            ...
            <div
              ...
              className="bg-white text-gray-900 rounded-3xl p-6 sm:p-8 shadow-xl border border-gray-200 transition-colors duration-300 relative print-text-dark font-sans"
            >
              {/* Document Stamp Indicator */}
              <div className="absolute top-28 right-8 border-4 border-primary-500 text-primary-500 font-display font-extrabold text-xl px-4 py-2 uppercase rounded-xl tracking-wider rotate-12 opacity-80 pointer-events-none">
                {docType}
              </div>

              {/* Letterhead Header Branding */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 text-xs text-gray-700 bg-slate-50 print-bg-gray p-4 rounded-2xl">
                <div>
                  <p className="mb-2"><span className="font-bold text-gray-500">{docType.toUpperCase()} NO:</span> <span className="font-mono font-bold text-gray-900">{quotationNo || 'N/A'}</span></p>
                  <p className="mb-2 flex items-center"><span className="font-bold text-gray-500 mr-2">DATE OF ISSUE:</span> <span className="font-semibold text-gray-900"><FontAwesomeIcon icon={faCalendarAlt} className="mr-1 text-gray-400" /> {date ? new Date(date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : 'N/A'}</span></p>
                  <p className="mb-0 flex items-center"><span className="font-bold text-gray-500 mr-2">VALID UNTIL:</span> <span className="font-bold text-primary-700"><FontAwesomeIcon icon={faClock} className="mr-1 text-primary-500" /> {getValidityDateString()}</span></p>
                </div>
                <div>
                  <p className="mb-2"><span className="font-bold text-gray-500">APPLIANCE:</span> <span className="font-bold text-gray-900">{applianceType} {applianceBrand ? `(${applianceBrand})` : ''}</span></p>
                  {applianceModel && <p className="mb-2"><span className="font-bold text-gray-500">MODEL NO:</span> <span className="font-semibold text-gray-900">{applianceModel}</span></p>}
                  {customerName && <p className="mb-2"><span className="font-bold text-gray-500">PREPARED FOR:</span> <span className="font-bold text-gray-900">{customerName}</span></p>}
                  {customerPhone && <p className="mb-0"><span className="font-bold text-gray-500">CONTACT:</span> <span className="font-semibold text-gray-900">{customerPhone} {customerEmail ? `| ${customerEmail}` : ''}</span></p>}
                </div>
              </div>

              {/* Subject Scope */}
              {subject && (
                <div className="mb-6 text-xs text-gray-800 border-l-4 border-primary-500 pl-4 py-1">
                  <span className="font-bold text-gray-500 block uppercase text-[10px] tracking-wider">Subject / Scope of Services:</span>
                  <span className="font-semibold text-slate-900 text-sm">{subject}</span>
                </div>
              )}
              ...
              {customerAddress && (
                <div className="mb-6 text-xs text-gray-700 bg-slate-50 print-bg-gray p-4 rounded-2xl">
                  <span className="font-bold text-gray-500 block mb-1">SITE / SERVICE ADDRESS:</span>
                ...
                <table className="w-full text-xs text-left">
                  <thead>
                    <tr className="border-b-2 border-slate-100 print-border-gray text-gray-500 font-bold">
                      <th className="pb-3 text-left">Description of Service / Part Proposal</th>
                  ...
                  <tbody className="divide-y divide-slate-100 print-divide-gray">
                    {items.map((item, i) => (
                      <tr key={i} className="text-gray-800">
                        <td className="py-3 font-semibold text-gray-950">{item.description || 'Appliance Repair Proposal Item'}</td>
                ...
                <div className="w-full sm:w-72 space-y-2.5 text-xs text-gray-700">
                  ...
                  {discount > 0 && (
                    <div className="flex justify-between text-emerald-600 font-semibold">
                      <span>Estimated Discount {discountType === 'percent' ? `(${discountValue}%)` : ''}:</span>
                  ...
                  {!isVisitingChargeWaived && visitingCharge > 0 && (
                    <div className="flex justify-between">
                      <span className="text-gray-500 font-semibold">Visiting Fee:</span>
                  ...
                  {otherCharges > 0 && (
                    <div className="flex justify-between">
                      <span className="text-gray-500 font-semibold">{otherChargesLabel || 'Other Charges'}:</span>
                      <span className="font-semibold text-gray-900">₹{otherCharges.toLocaleString('en-IN')}</span>
                    </div>
                  )}

                  {gstRate > 0 && (
                    <div className="flex justify-between text-gray-600">
                      <span className="font-semibold text-gray-500">Estimated GST ({gstRate}%):</span>
                  ...
                  <div className="flex justify-between text-sm pt-2.5 border-t border-slate-100 print-border-gray font-display font-bold text-slate-900">
                    ...
                    <span className="text-base text-primary-600 font-extrabold">₹{Math.round(grandTotal).toLocaleString('en-IN')}</span>
                  </div>
                </div>
              </div>

              {/* Terms and Acceptance signatures */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-[10px] text-gray-500 leading-relaxed border-t border-slate-100 print-border-gray pt-6">
                <div>
                  <h5 className="font-bold text-gray-700 uppercase mb-2">Terms &amp; Conditions</h5>
                  <p className="whitespace-pre-wrap leading-relaxed">{notes}</p>
                </div>
                <div className="grid grid-cols-2 gap-4 mt-6 md:mt-0 pt-4 md:pt-0">
                  <div className="flex flex-col justify-end items-center text-center">
                    <div className="h-10 w-full border-b border-gray-300 print-border-gray"></div>
                    <p className="mt-2 font-bold text-gray-600 uppercase tracking-wider text-[8px]">Client Acceptance</p>
                    <p className="text-gray-400 text-[8px]">Sign &amp; Date</p>
                  </div>
                  <div className="flex flex-col justify-end items-center text-center">
                    <div className="h-10 w-full border-b border-gray-300 print-border-gray"></div>
                    <p className="mt-2 font-bold text-gray-600 uppercase tracking-wider text-[8px]">Authorised Signature</p>
